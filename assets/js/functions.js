...
var listenersFunctions = {
    ...
    showIconDiscussion: () => {
        ...
        for (let index = 0; index < contactItems.length; index++) {
            ...
            contactItem.addEventListener('mouseover', () => {
                document.querySelectorAll('.contact-time-icon >img')[index] ?  document.querySelectorAll('.contact-time-icon >img')[index].classList.add('arrow-active') : null ;
            });
            contactItem.addEventListener('mouseout', () => {
                document.querySelectorAll('.contact-time-icon >img')[index] ?  document.querySelectorAll('.contact-time-icon >img')[index].classList.remove('arrow-active') : null;
            });
        }
    },
    showIconMessage: () => {
    ...
    TransformSearchIconContacts: () => {
        document.querySelector('.bars-icon.contacts input').addEventListener('click', () => {
            document.querySelectorAll('.search-icon.contacts')[0].classList.toggle('transform');
        });
        document.querySelectorAll('.search-icon.contacts')[0].addEventListener('click', (ev) => {
            ev.target.classList.toggle('transform')
            document.querySelector('.bars-icon.contacts input').value = '';
            listenersFunctions.searchMessages("");
        });
    },
    getInputSearchContacts: () => {
        document.querySelector('.bars-icon.contacts input').addEventListener('input', (ev) => {
            let searchTerm = ev.target.value.trim();
            listenersFunctions.searchMessagesContacts(searchTerm);
        });
        document.querySelector('.bars-icon.contacts input').value = '';
    },

    searchMessagesContacts: (searchTerm) => {
        let resultFound = false;
...
var setupFunctions = () => {
    ...
    listenersFunctions.getInputSearchContacts();
    listenersFunctions.TransformSearchIconContacts();
